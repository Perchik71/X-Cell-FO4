...
namespace xc
{
	...
	namespace own_mm
	{
		...
		public:
			...
			inline static void* impl_calloc(size_t count, size_t size)
			{
				return alloc(count * size, 0);
			}

			inline static void* impl_malloc(size_t size)
			{
				return alloc(size, 0);
			}

			inline static void* impl_aligned_malloc(size_t size, size_t alignment)
			{
				return alloc(size, alignment, true);
			}

			inline static void* impl_realloc(void* memory, size_t size)
			{
				void* newMemory = nullptr;

				if (size > 0)
				{
					newMemory = alloc(size, 0, false);
			...
			inline static void impl_free(void* block)
			{
				dealloc(block);
			}

			inline static void impl_aligned_free(void* block)
			{
				dealloc(block);
			}

			inline static size_t impl_msize(void* block)
			{
				return msize(block);
			}
		};
	...
	bool patch_memory::run() const
	{
		...
		else if (g_plugin->get_runtime_version() == RUNTIME_VERSION_1_10_984)
		{
			...
			_FATALERROR("The patch has not been fully installed, as the mod does not know the game");

		os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE = g_plugin->get_settings()->read_uint("additional", "scaleform_page_size", 256 /* 256 Kb */);
		os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE = g_plugin->get_settings()->read_uint("additional", "scaleform_heap_size", 512 /* 512 Mb */);

		os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE = min(os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE, (UInt32)2 * 1024);
		os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE = (os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE + 7) & ~7;
		os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE = min(os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE, (UInt32)2 * 1024);
		os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE = (os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE + 7) & ~7;

		_MESSAGE("BSScaleformSysMemMapper (Page: %u Kb, Heap: %u Mb)",
			os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE, os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE);

		os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE *= 1024;
		os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE *= 1024 * 1024;

		if (g_plugin->get_runtime_version() == RUNTIME_VERSION_1_10_163)
		{
			auto vtable = (uintptr_t*)(g_plugin->get_base() + 0x2EB92C8);
			scope_relocate_al lock((LPVOID)vtable, 0x40);
			vtable[0] = (uintptr_t)os_mm::detail::BSScaleformSysMemMapper::get_page_size;
			vtable[1] = (uintptr_t)os_mm::detail::BSScaleformSysMemMapper::init;
			vtable[2] = (uintptr_t)os_mm::detail::BSScaleformSysMemMapper::release;
			vtable[3] = (uintptr_t)os_mm::detail::BSScaleformSysMemMapper::alloc;
			vtable[4] = (uintptr_t)os_mm::detail::BSScaleformSysMemMapper::free;

			patch_mem((g_plugin->get_base() + 0x211214B), (UInt8*)&os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE, 4);
			patch_mem((g_plugin->get_base() + 0x2112151), (UInt8*)&os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE, 4);
		...
		else if (g_plugin->get_runtime_version() == RUNTIME_VERSION_1_10_984)
		{
			...
			patch_mem((g_plugin->get_base() + 0x19FF5D9), (UInt8*)&os_mm::detail::BSScaleformSysMemMapper::PAGE_SIZE, 4);
			patch_mem((g_plugin->get_base() + 0x19FF5E4), (UInt8*)&os_mm::detail::BSScaleformSysMemMapper::HEAP_SIZE, 4);
		...
		if (g_plugin->get_settings()->read_bool("additional", "use_os_memory_manager", false))
		{
			// Replacement of all functions of the standard allocator.

			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "realloc", (uintptr_t)&realloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "calloc", (uintptr_t)&calloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "_aligned_malloc", (uintptr_t)&_aligned_malloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "malloc", (uintptr_t)&malloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "_aligned_free", (uintptr_t)&_aligned_free);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "free", (uintptr_t)&free);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "_msize", (uintptr_t)&_msize);
			...
			{
				// 163
				detour_jump((g_plugin->get_base() + 0x1B0EFD0), (uintptr_t)&os_mm::detail::BGSMemoryManager::alloc);
				detour_jump((g_plugin->get_base() + 0x1B0F2E0), (uintptr_t)&os_mm::detail::BGSMemoryManager::dealloc);
				detour_jump((g_plugin->get_base() + 0x1B0E7D0), (uintptr_t)&os_mm::detail::BGSMemoryManager::msize);
				detour_jump((g_plugin->get_base() + 0x1B13F70), (uintptr_t)&os_mm::detail::BGSScrapHeap::alloc);
				detour_jump((g_plugin->get_base() + 0x1B14580), (uintptr_t)&os_mm::detail::BGSScrapHeap::dealloc);
				detour_jump((g_plugin->get_base() + 0x1E21B10), (uintptr_t)&os_mm::detail::bhkThreadMemorySource::__ctor__);	// bhkThreadMemorySource init
			}
			else if (g_plugin->get_runtime_version() == RUNTIME_VERSION_1_10_984)
			{
				// 984
				detour_jump((g_plugin->get_base() + 0x153D7D0), (uintptr_t)&os_mm::detail::BGSMemoryManager::alloc);
				detour_jump((g_plugin->get_base() + 0x153DC30), (uintptr_t)&os_mm::detail::BGSMemoryManager::dealloc);
				detour_jump((g_plugin->get_base() + 0x153DA30), (uintptr_t)&os_mm::detail::BGSMemoryManager::realloc);
				detour_jump((g_plugin->get_base() + 0x153CFA0), (uintptr_t)&os_mm::detail::BGSMemoryManager::msize);
				detour_jump((g_plugin->get_base() + 0x1542010), (uintptr_t)&os_mm::detail::BGSScrapHeap::alloc);
				detour_jump((g_plugin->get_base() + 0x15425E0), (uintptr_t)&os_mm::detail::BGSScrapHeap::dealloc);
				detour_jump((g_plugin->get_base() + 0x17D9DF0), (uintptr_t)&os_mm::detail::bhkThreadMemorySource::__ctor__);	// bhkThreadMemorySource init	
			}
		}
		else
		{
			...
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "realloc", (uintptr_t)&own_mm::memory_manager::impl_realloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "calloc", (uintptr_t)&own_mm::memory_manager::impl_calloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "_aligned_malloc", (uintptr_t)&own_mm::memory_manager::impl_aligned_malloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "malloc", (uintptr_t)&own_mm::memory_manager::impl_malloc);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "_aligned_free", (uintptr_t)&own_mm::memory_manager::impl_aligned_free);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "free", (uintptr_t)&own_mm::memory_manager::impl_free);
			patch_iat(base, "API-MS-WIN-CRT-HEAP-L1-1-0.DLL", "_msize", (uintptr_t)&own_mm::memory_manager::impl_msize);
			...
			{
				// 163
				detour_jump((g_plugin->get_base() + 0x1B0EFD0), (uintptr_t)&own_mm::detail::BGSMemoryManager::alloc);
				detour_jump((g_plugin->get_base() + 0x1B0F2E0), (uintptr_t)&own_mm::detail::BGSMemoryManager::dealloc);
				detour_jump((g_plugin->get_base() + 0x1B0E7D0), (uintptr_t)&own_mm::detail::BGSMemoryManager::msize);
				detour_jump((g_plugin->get_base() + 0x1B13F70), (uintptr_t)&own_mm::detail::BGSScrapHeap::alloc);
				detour_jump((g_plugin->get_base() + 0x1B14580), (uintptr_t)&own_mm::detail::BGSScrapHeap::dealloc);
				detour_jump((g_plugin->get_base() + 0x1E21B10), (uintptr_t)&own_mm::detail::bhkThreadMemorySource::__ctor__);	// bhkThreadMemorySource init
			}
			else if (g_plugin->get_runtime_version() == RUNTIME_VERSION_1_10_984)
			{
				// 984
				detour_jump((g_plugin->get_base() + 0x153D7D0), (uintptr_t)&own_mm::detail::BGSMemoryManager::alloc);
				detour_jump((g_plugin->get_base() + 0x153DC30), (uintptr_t)&own_mm::detail::BGSMemoryManager::dealloc);
				detour_jump((g_plugin->get_base() + 0x153DA30), (uintptr_t)&own_mm::detail::BGSMemoryManager::realloc);
				detour_jump((g_plugin->get_base() + 0x153CFA0), (uintptr_t)&own_mm::detail::BGSMemoryManager::msize);
				detour_jump((g_plugin->get_base() + 0x1542010), (uintptr_t)&own_mm::detail::BGSScrapHeap::alloc);
				detour_jump((g_plugin->get_base() + 0x15425E0), (uintptr_t)&own_mm::detail::BGSScrapHeap::dealloc);
				detour_jump((g_plugin->get_base() + 0x17D9DF0), (uintptr_t)&own_mm::detail::bhkThreadMemorySource::__ctor__);	// bhkThreadMemorySource init	
			}
		}
